                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-hover singcms-table">
                        ...
                        <?php if(is_array($menus)): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><tr>
                                <td><input size="4" type="text" name="listorder[<?php echo ($menu["menu_id"]); ?>]" value="<?php echo ($menu["listorder"]); ?>"/></td><!--排序的输入框-->
                                <td><?php echo ($menu["menu_id"]); ?></td><!--id-->
                                <td><?php echo ($menu["name"]); ?></td><!--菜单管理-->
                                <td><?php echo ($menu["m"]); ?></td><!--模块名-->
                                <td><?php echo (getMenuType($menu["type"])); ?></td><!--数据类型-->
                                <td><?php echo (status($menu["status"])); ?></td><!--状态-->
                                <td>
...
<script>
    ...
    var SCOPE = {
        ...
        'edit_url' : '/admin.php?c=menu&a=edit',  <!-- 编辑模式下的跳转 -->
